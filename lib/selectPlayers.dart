...
class SelectPlayersState extends State<SelectPlayersPage> {
  ...
  void saveRoster() {
    for (String id in selected) {
      LineupEntry newLineupEntry = LineupEntry(lineupID: lineupID, playerID: id, rotation: Random().nextInt(9));
                    ...
                    return ListView.builder(
                        ...
                        itemBuilder: ((context, index) {
                          return Card(
                            ...
                            child: ListTile(
                                tileColor: selected.contains(snapshot.data![index].id)
                                    ? const Color(0x9900E5FF)
                                    : const Color.fromARGB(150, 173, 32, 32),
                                onTap: () {
                                  if (selected.contains(snapshot.data![index].id)) {
                                ...
                                leading: selected.contains(snapshot.data![index].id)
                                    ? selected.toList().indexOf(snapshot.data![index].id) < 6
                                        ? Text((selected.toList().indexOf(snapshot.data![index].id) + 1).toString())
                                        : const Text("Sub")
                                    : const Text(""),
                                title: Text(snapshot.data![index].name, style: const TextStyle(fontSize: 24)),
